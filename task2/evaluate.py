...
FINAL_OUTPUT_DIR = "submission" # 최종 .pkl 파일이 저장될 위치
LPIPS_SAMPLE_PAIRS = 1000
# ----------------

# main.py에서 모델 정의와 PARAMS를 그대로 가져옵니다.
# -----------------------------------------------------------------------------
PARAMS = {
...
class Generator(nn.Module):
    ...
    def forward(self, noise, labels):
        ...
        return torch.nn.functional.interpolate(img, size=(PARAMS['img_height'], PARAMS['img_width']), mode='bilinear', align_corners=False)
# -----------------------------------------------------------------------------
...
if __name__ == "__main__":
    ...
    if not checkpoint_paths:
        print(f"오류: '{CHECKPOINTS_DIR}' 폴더에서 체크포인트 파일을 찾을 수 없습니다.")
        exit()

    # --- 1단계: 모든 체크포인트를 내부적으로 평가하여 최고의 모델 찾기 ---
    print(f"\n--- 1단계: 총 {len(checkpoint_paths)}개의 체크포인트에서 최고의 모델을 찾습니다 ---")
    lpips_model = lpips.LPIPS(net='vgg').to(device)
    real_images_cache = {}
    all_results = []

    for cp_path in tqdm(checkpoint_paths, desc="전체 체크포인트 평가 중"):
    ...
    print(f"\n🎉 최고의 모델을 찾았습니다: Epoch {best_model_info['epoch']} (평균 FID: {best_model_info['avg_fid']:.4f})")

    # --- 2단계: 최고의 모델로 최종 .pkl 파일 생성 ---
    print(f"\n--- 2단계: Epoch {best_model_info['epoch']} 모델로 최종 pkl 파일을 생성합니다 ---")
    ...
    # --- 3단계: 생성된 최종 파일에 대한 상세 평가 결과 보고 ---
    print(f"\n--- 3단계: 최종 생성된 파일에 대한 상세 평가 결과입니다 ---")
    final_eval_results = []
    for cond in ['a', 'b', 'c']:
        real_imgs = load_real_images(REAL_DATA_PATHS, cond, real_images_cache)
