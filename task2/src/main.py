...
from torch.optim.lr_scheduler import StepLR # <--- 스케줄러 import
import numpy as np
import pickle
import os
...
def load_and_prepare_data(data_paths):
    ...
    images_tensor = torch.from_numpy(images_np).permute(0, 3, 1, 2); images_tensor = (images_tensor / 127.5) - 1.0
    return TensorDataset(images_tensor, torch.LongTensor(all_labels))

# --- 생성자(Generator) 모델 정의 (변경 없음) ---
class Generator(nn.Module):
    def __init__(self):
        super(Generator, self).__init__(); self.label_embedding = nn.Embedding(PARAMS['n_classes'], PARAMS['latent_dim']); self.init_size_h, self.init_size_w = 8, 16; self.l1 = nn.Sequential(nn.Linear(PARAMS['latent_dim'] * 2, 128 * self.init_size_h * self.init_size_w)); self.conv_blocks = nn.Sequential(nn.BatchNorm2d(128), nn.Upsample(scale_factor=2), nn.Conv2d(128, 128, 3, 1, 1), nn.BatchNorm2d(128, 0.8), nn.LeakyReLU(0.2, inplace=True), nn.Upsample(scale_factor=2), nn.Conv2d(128, 64, 3, 1, 1), nn.BatchNorm2d(64, 0.8), nn.LeakyReLU(0.2, inplace=True), nn.Upsample(scale_factor=2), nn.Conv2d(64, 32, 3, 1, 1), nn.BatchNorm2d(32, 0.8), nn.LeakyReLU(0.2, inplace=True), nn.Upsample(scale_factor=2), nn.Conv2d(32, PARAMS['channels'], 3, 1, 1), nn.Tanh())
    def forward(self, noise, labels):
        label_input = self.label_embedding(labels); gen_input = torch.mul(label_input, noise); combined_input = torch.cat((gen_input, noise), dim=1); out = self.l1(combined_input); out = out.view(out.shape[0], 128, self.init_size_h, self.init_size_w); img = self.conv_blocks(out); return torch.nn.functional.interpolate(img, size=(PARAMS['img_height'], PARAMS['img_width']), mode='bilinear', align_corners=False)

# --- 판별자(Discriminator) 모델 정의 (구조 강화) ---
class Discriminator(nn.Module):
    def __init__(self):
        super(Discriminator, self).__init__()
        self.label_embedding = nn.Embedding(PARAMS['n_classes'], PARAMS['channels'] * PARAMS['img_height'] * PARAMS['img_width'])
...
if __name__ == "__main__":
    ...
    for epoch in range(PARAMS['epochs']):
        ...
        for i, (imgs, labels) in progress_bar:
            ...
            progress_bar.set_description(f"[Epoch {epoch+1}/{PARAMS['epochs']}] [D loss: {d_loss.item():.4f}] [G loss: {g_loss.item():.4f}]")
        
        # --- 각 에포크가 끝날 때마다 스케줄러를 업데이트 ---
        scheduler_G.step()
        scheduler_D.step()
        ...
        if (epoch + 1) % PARAMS['checkpoint_interval'] == 0:
            ...
            print(f"Epoch {epoch+1} 모델 체크포인트 저장 완료.")
